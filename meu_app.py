...
# ==========================================
# CÉREBRO DA DISTRIBUIÇÃO EM LOTE
# ==========================================
def construir_mapa_lote_offline(lat_c, lon_c, lat_t, lon_t, nome, codigo, num_trafo, obs, font_size=8, font_style="Arial"):
    ...
    # 1. GERAR A ROTA DE ARRUAMENTO AUTOMATICAMENTE
    ...
    # Traçado visual premium da rede
    ...
    # ==============================================================
    # 2. LÓGICA DE DISTRIBUIÇÃO ESPACIAL (DINÂMICA)
    # ==============================================================
    # Calcula a direção do vetor Trafo -> Cliente
    d_lat = fl_lat_c - fl_lat_t
    d_lon = fl_lon_c - fl_lon_t
    
    # Afasta o card do Cliente para "longe" do Trafo (Ajustado para ficar mais perto)
    off_lat_c = 0.00015 if d_lat >= 0 else -0.00015
    off_lon_c = 0.00025 if d_lon >= 0 else -0.00025
    
    # Afasta o card do Trafo para "longe" do Cliente (Ajustado para ficar mais perto)
    off_lat_t = -0.00015 if d_lat >= 0 else 0.00015
    off_lon_t = -0.00025 if d_lon >= 0 else 0.00025

    # Ajuste de Observação no topo
    ...
    # Limites da câmera com margem ampla para não cortar cards
    limites_camera = [
        [fl_lat_c + 0.001, fl_lon_c + 0.001], 
        [fl_lat_c - 0.001, fl_lon_c - 0.001],
        [fl_lat_t + 0.001, fl_lon_t + 0.001], 
        [fl_lat_t - 0.001, fl_lon_t - 0.001],
...
# ==========================================
# Offsets padrão dinâmicos (Ajustados para ficarem mais próximos conforme pedido)
...
def construir_mapa_camadas(is_print=False):
    ...
    if is_print and st.session_state.get("map_center"):
        ...
        centro_mapa = [fl_lat_c if 'fl_lat_c' in locals() else -4.512536, fl_lon_c if 'fl_lon_c' in locals() else -44.469452]
        zoom_mapa = 17
    ...
    if 'lat_c' in locals() and str(lat_c).strip() and str(lon_c).strip():
        ...
        html_cliente = f'<div style="{card_style}">{nome if "nome" in locals() else ""}<br>MD: {codigo if "codigo" in locals() else ""}{info_tel}</div>'
    ...
    for idx, marc in enumerate(st.session_state.marcadores_extras):
        ...
        # Ajustado para ficar mais perto da simbologia
    ...
    if 'obs' in locals() and str(obs).strip():
...
with coluna_painel:
                ...
                if st.button("🚀 GERAR LOTE COMPLETO (ZIP)", use_container_width=True):
                    ...
                    with zipfile.ZipFile(zip_buffer, "w") as zf:
                        for idx, row in df_lote.iterrows():
                            ...
                            # CORREÇÃO: Gerar TXT e incluir no ZIP
    ...
    with aba_exportar:
        # CORREÇÃO: Exportar imagem e TXT juntos em um ZIP para exportação manual
        if st.button("📸 GERAR CROQUI COMPLETO (PNG + TXT)", use_container_width=True):
            with st.spinner("Gerando..."):
                # Gerar PNG
                print_bytes = gerar_print_mapa(construir_mapa_camadas(is_print=True).get_root().render())
